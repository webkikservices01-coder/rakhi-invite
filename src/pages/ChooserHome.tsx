...
import rakhiImg from "@/assets/hero-tying.jpg";
import weddingImg from "@/verticals/wedding/assets/monument-taj.jpg";
import webkikLogo from "@/assets/webkik-logo.png";
...
const quickLinks = [
  { label: "About Us", to: "/rakhi/about" },
  { label: "Contact Us", to: "/rakhi/contact" },
  { label: "FAQs", to: "/rakhi/faqs" },
  { label: "Privacy Policy", to: "/rakhi/privacy" },
  { label: "Terms & Conditions", to: "/rakhi/terms" },
  { label: "Refund Policy", to: "/rakhi/refund-policy" },
  { label: "Cancellation Policy", to: "/rakhi/cancellation-policy" },
];
...
export default function ChooserHome() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[oklch(0.97_0.02_85)] via-[oklch(0.94_0.04_70)] to-[oklch(0.9_0.06_55)]">
      ...
      {/* Hero Section */}
      <section className="mx-auto max-w-5xl px-6 pt-12 pb-8 text-center sm:pt-16">
        <h1 className="font-display text-5xl sm:text-7xl font-bold leading-[0.95]">
          One studio.<br />Every invitation.
        </h1>
        <p className="mx-auto mt-6 max-w-xl text-lg opacity-75">
          Personalized, shareable invite cards for the moments that matter — pick what you're celebrating below.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-6 pb-16 sm:pb-24">
        <div className="grid gap-6 sm:grid-cols-2">
          <ChooserCard
            onClick={() => navigate("/rakhi")}
            icon={<Heart className="text-rose-500" />}
            eyebrow="Raksha Bandhan"
            title="Send a Rakhi Invitation"
            desc="60+ templates across Silver, Gold & Platinum — with music, real family photos, and an AI companion."
            img={rakhiImg}
            cta="Explore Rakhi Templates"
          />
          <ChooserCard
            onClick={() => navigate("/wedding")}
            icon={<Gem className="text-fuchsia-500" />}
            eyebrow="Weddings, worldwide"
            title="Send a Wedding Invitation"
            desc="65 cinematic templates set against the Taj, Burj Khalifa, Eiffel Tower and more — with RSVP built in."
            img={weddingImg}
            cta="Explore Wedding Templates"
          />
        </div>
      </section>
...
function ChooserCard({
  ...
  return (
    <button
      onClick={onClick}
      className="group relative overflow-hidden rounded-[2rem] text-left shadow-soft transition-all hover:-translate-y-1 hover:shadow-2xl"
    >
      <div className="relative aspect-[5/4] overflow-hidden">
        <img src={img} alt="" className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
      </div>
      <div className="absolute inset-x-0 bottom-0 p-6 text-white">
        <div className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-white/20 px-3 py-1 text-[10px] font-bold uppercase tracking-widest backdrop-blur">
          {icon} {eyebrow}
        </div>
        <div className="font-display text-3xl font-bold drop-shadow-lg">{title}</div>
        <p className="mt-1.5 max-w-sm text-sm opacity-90">{desc}</p>
        <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-black group-hover:gap-3 transition-all">
      ...
      <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
        ...
        <div className="mt-16 flex flex-col items-center justify-between gap-4 border-t border-[#E70C65]/15 pt-8 sm:flex-row">
          ...
          <p className="shimmer-text bg-gradient-to-r from-[#E70C65] via-[#9F1C44] to-[#E70C65] bg-clip-text text-sm font-semibold text-transparent">
            A digital card platform by {COMPANY.name}
          </p>
